...
class Parede:
    ...
    def render_reflection(self, lista_de_exebicao, cuboAranha, camera_position):
        ...
        # Ajustar o frustum com base no tamanho da
        near_plane = 0.1
        far_plane = 5000.0
        ...
        min_distance = 1.0  # Distância mínima permitida da câmera ao espelho
        distance_to_mirror = np.dot(camera_position - self.position, self.direction)

        if distance_to_mirror < min_distance:
            # Ajustar a posição da câmera para a distância mínima
            camera_position = self.position + self.direction * min_distance
        ...
        gluLookAt(reflection_position[0], reflection_position[1], reflection_position[2],
                  look_at_position[0]+10, look_at_position[1], look_at_position[2],
                  0, 1, 0)
