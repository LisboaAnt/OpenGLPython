...
def plot_polygon():
    glColor(0, 1, 0, 1)
    glBegin(GL_QUAD_STRIP) #GL_QUAD_STRIP, GL_QUADS , GL_TRIANGLES, GL_TRIANGLE_LOOP
    for p in points:
        glVertex2f(p[0], p[1])
    glEnd()
    glColor(1, 0, 0, 1)
...
def plot_polygon():
    ...
    glBegin(GL_TRIANGLE_STRIP) #GL_QUAD_STRIP, GL_QUADS , GL_TRIANGLES, GL_TRIANGLE_LOOP
    for p in points:
        glVertex2f(p[0], p[1])
    glEnd()
